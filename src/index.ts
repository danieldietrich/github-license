 ...
 */
export = async function githubLicense(spdxIdentifier?: string): Promise<string> {
    return spdxIdentifier
        ? (await _call_github_api<{ body: string }>()).body
        : (await _call_github_api<[{ spdx_id: string, name: string }]>())
            .map(({ spdx_id, name }) => `${spdx_id}\t${name}`)
            .sort()
            .join('\n');

    // tslint:disable-next-line no-any
    async function _call_github_api<T>(): Promise<T> {
